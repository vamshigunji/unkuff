// Seed script to populate test jobs for drag-and-drop testing
// Run with: npx tsx scripts/seed-test-jobs.ts
...
async function seedTestJobs() {
    ...
    for (const job of testJobs) {
        const hash = `test-${job.company.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`;
        ...
        await db.insert(jobs).values({
            ...
            postedAt: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000), // Random within last 7 days
        });
